...
const tokenAddress: any = {
  ...
  BSC: {
    ...
    TTUSD: "0x13a5bcff72b024619df4d62093941c382b6d6348",
    DAI: "0x1AF3F329e8BE154074D8769D1FFa4eE058B1DBc3",
  },
};

const chainToken: any = {
  BSC: 'BNB',
  BKC: 'KUB'
}

const tokenCoingeckoId: any = {
  BKC: {
    KUB: "bitkub-coin",
    WKUB: "bitkub-coin",
    DAI: "dai",
...
export const getTokenBalance = async (provider: any, walletAddress: string) => {
  ...
  fetch("https://api.coingecko.com/api/v3/coins/bitkub-coin")
    .then((res) => res.json())
    .then(
      (result) => {
  ...
  const pushTokenbalance = async (
    contract: ethers.Contract,
    chain: string,
    tokenName: string,
    walletAddress: string
  ) => {
    tokenBalance[chain][tokenName] = ethers.utils.formatUnits(
  ...
  for (let chain in tokenAddress) {
    for (let tokenName in tokenAddress[chain]) {
      let contract = new ethers.Contract(
        tokenAddress[chain][tokenName],
        abi,
        provider[chain]
      );
      let balance: number = parseFloat(
        ethers.utils.formatUnits(await contract.balanceOf(walletAddress))
      );
        ...
        PromiseBalance.push({
          ...
          price: parseFloat(
            (
              await (
                await fetch(
                  `https://api.coingecko.com/api/v3/coins/${tokenCoingeckoId[chain][tokenName]}`
                )
              ).json()
  ...
  for (let chain in chainToken){
      let tokenName: string = chainToken[chain];
      let balance:number = parseFloat(ethers.utils.formatUnits(await provider[chain].getBalance(walletAddress)))
      PromiseBalance.push({
        symbol: tokenName,
        chain: chain,
        balance: balance,
  ...
  console.log("Promisebalance", PromiseBalance);
